...
**Event:** A particular occurrence of a natural or man-made disaster. Events can refer to real-life historical events that have actually occurred or can also refer to hypothetical or synthetic events 
...
**Exposure** Definition 1: Exposure is one of the key components of a catastrophe model, along with hazard and vulnerability. Exposure refers to the assets you want to insure. The exposure(s) can be defined as a 
specific location(s), a list of assets/buildings or an area that is to be protected by the insurance cover. This module interacts with the hazard and vulnerability modules eventually producing potential financial 
losses to the exposure.

Definition 2: An insurance term that refers to a method of quantifying the expected insurance loss to a cover based on the underlying assets being covered. Typically the way the exposure price is calculated will 
differ based on the type of asset being covered e.g. a building's construction type.
...
previous cyclones. These wind speeds are used as a measure of the relative "hazard" or level of risk from a cyclone event at each point. This module interacts with the exposure and vulnerability modules 
eventually producing potential financial losses to the exposure.
...
IBTrACS is the main source of hazard data for the Risk Explorer at present and is compiled by US government body, NOAA (National Oceanic and Atmospheric Administration).
...
**Latitude-Longitude:** This term is often abbreviated to lat-long. Latitude and longitude can be thought of as a global coordinate system, any location on the earth's surface can be defined by a particular latitude
and longitude. Longitude can be thought of as how far east or west a location is, while latitude represents how far north or south a location is. Longitude values range from 180 degrees East (+180) to 180 degrees West (-180), 
...
**Maximum Insured/Limit:** The maximum amount that can be paid out by a cover when an event occurs. Note that the maximum insured does not represent the total potential amount insured, as the maximum insured could 
...
interchangeably here in order to expose users to the more intuitive definition as well as the more standard market terminology.
...
the likelihood of seeing losses/payouts for different covers regardless of the financial amount. E.g. Consider a cover with only one trigger of USD 100,000. The simulated loss is 20,000 which is 20% of the maximum. 
This 20% gives us an idea of the frequency of events hitting the cover, i.e we would expect a loss roughly 20% or every 1 in 5 years. A separate cover with a trigger of 1,000,000 also has a simulated loss of 20,000 
...
a reinstatement you would receive the full amount of 60% for the second event as the cover would reinstate so you are entitled to an additional maximum payout over the course of the year. If you had no 
...
is capped so as not to exceed the maximum insured. 
The pricing for a cover with multiple reinstatements will differ to that of a single event cover.
...
is the probability of occurrence in any given year. A 10-year return period means there is a 1 in 10 (10%) chance of it happening in any given year. For remote events, return periods can be considered the reciprocals
...
In the case of version 1 of the Risk Explorer, what we are trying to simulate is the insurance losses at a randomly sampled location over the history of the IBTrACS data-set. As such, 
...
**Saffir-Simpson Category:** Each category represents a level of hurricane intensity on the Saffir-Simpson scale which is commonly used by meteorologists and the insurance industry. The Saffir-Simpson scale
...
weight simulations a long way from the exposure. However, it can be useful for getting an idea of how much impact the weighting function is having on the weighted simulation loss by comparing it with this measure.
...
the insurance structure itself are essentially the same thing. This module interacts with the exposure and hazard modules eventually producing potential financial losses to the exposure.
...
outputs from the simulation approach. See the "How does the Simulation Methodology Work?" in the FAQs section for a more detailed discussion of how this is calculated.
